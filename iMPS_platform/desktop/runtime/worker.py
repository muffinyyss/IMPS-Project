...
"""Run the verified AgenticAI detector in the portable desktop sidecar."""
...
def artifact_version(model_dir: Path) -> str:
    manifest_path = model_dir / "manifest.json"
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        raise RuntimeError("Packaged model manifest is missing or invalid") from exc
    version = manifest.get("artifactVersion") if isinstance(manifest, dict) else None
    if not isinstance(version, str) or not version:
        raise RuntimeError("Packaged model manifest has no artifactVersion")
    for name in ("lstm_ae.npz", "gru_fore.npz"):
        expected = (manifest.get("files") or {}).get(name, {}).get("sha256")
        path = model_dir / name
        if not path.is_file() or expected != sha256_file(path):
            raise RuntimeError(f"Packaged model artifact failed validation: {name}")
    return version
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    tshark = args.tshark.resolve(strict=True)
    ...
    extracted_events = int(extract(str(input_path), str(telemetry_path), tshark))
    update_progress(
        args.progress,
        "sessionizing",
        42,
    ...
    detector = None
    model_version = artifact_version(model_dir)
    if extracted_events:
