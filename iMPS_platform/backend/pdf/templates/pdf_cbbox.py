...
def _draw_summary_checklist(pdf: FPDF, base_font: str, x: float, y: float, summary_check: str):
    ...
    for index, (label, checked) in enumerate(options):
        start_x = x + index * 23
        _draw_check(pdf, start_x, y, CHECKBOX_SIZE, checked)
        pdf.set_xy(start_x + CHECKBOX_SIZE + 2, y - 0.5)
        pdf.cell(18, LINE_H, label, align="L")
    return y + LINE_H
...
def make_pm_report_html_pdf_bytes(doc: dict, lang: str = "th") -> bytes:
    ...
    # แสดงข้อมูลงานใต้หัวเรื่อง — เลื่อนเส้นแบ่งคอลัมน์ให้ตรงกับเส้นแบ่ง Item | Result ของตารางด้านล่าง
    y = _draw_job_info_block(pdf, base_font, x0, y, page_w, station_name, pm_date, label_station, label_pm_date, col_split=EDGE_ALIGN_FIX + ITEM_W)
