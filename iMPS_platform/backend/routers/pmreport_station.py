...
from config import (
    normalize_pm_date, stationPMReportDB, stationPMUrlDB,
...
async def _latest_pm_date_from_stationpmreport(station_id: str) -> dict | None:
    ...
    pipeline = [
        ...
        {"$sort": {"_ts": -1, "_id": -1}},
        {"$limit": 1},
        {"$project": {"_id": 1, "pm_date": 1, "createdAt": 1}}
    ]
...
async def _stationpmreport_latest_core(station_id: str, current: UserClaims):
    """Get latest PM report info for a Station by station_id"""
    _validate_station_id_th(station_id)

    station = await _get_station_by_id(station_id)

    pm_latest = await _latest_pm_date_from_stationpmreport(station_id)
    pm_date = pm_latest.get("pm_date") if pm_latest else None

    ts_raw = (pm_latest.get("createdAt") if pm_latest else None) or station.get("createdAt")
    ...
    return {
        ...
        "timestamp_th": ts_th,
        "source": "stations + stationPMReportDB",
    }
...
async def stationpmreport_list(
    station_id: str = Query(...),
...
@router.get("/stationpmreport/latest/{station_id}")
async def stationpmreport_latest_by_path(
    station_id: str = Path(..., description="Station ID"),
    current: UserClaims = Depends(get_current_user),
):
    """Get latest PM info for a Station by station_id (path param)"""
    return await _stationpmreport_latest_core(station_id, current)
...
async def stationpmreport_post_submit(
    body: stationPMPostIn,
    current: UserClaims = Depends(get_current_user),
):
...
async def stationpmreport_upload_post_photos(
    report_id: str,
    station_id: str = Form(...),
    group: str = Form(...),
    files: List[UploadFile] = File(...),
    ...
    """Upload Post-PM photos for a Station report"""
    # ตรวจสิทธิ์สถานีก่อนแตะข้อมูล — เดิมเช็คแค่ว่า field ตรงกับ document
    # ซึ่งผู้เรียกคุมได้ทั้งคู่ จึงยิงข้ามสถานีได้
    assert_station_access(current, station_id)
    if not PHOTO_GROUP_PATTERN.match(group):
...
async def stationpmreport_approve(
    ...
    maximo_result = await pm_maximo_out.safe_sync_closed(
        coll, oid, fresh, memo=f"closed by {current.username}", kind="station"
    )
...
async def stationpmurl_upload_files(
    ...
    """Upload PM PDF files for a Station"""
    # ตรวจสิทธิ์สถานีก่อนแตะข้อมูล — เดิมเช็คแค่ว่า field ตรงกับ document
    # ซึ่งผู้เรียกคุมได้ทั้งคู่ จึงยิงข้ามสถานีได้
    assert_station_access(current, station_id)
    ...
    for f in files:
        ...
        urls.append(f"/uploads/stationpmurl/{station_id}/{pm_date}/{fname}")
...
async def stationpmurl_list(
    ...
    """List PM URL documents for a Station"""
    coll = get_stationpmurl_coll_upload(station_id)
    skip = (page - 1) * pageSize
...
@router.post("/stationpmreport/migrate/has-photos")
...
@router.get("/stationpmreport/{report_id}/photos/zip")
async def download_photos_zip(
    ...
    for group, items in photos_pre.items():
        for item in (items or []):
            ...
            if not fname:
                continue
            disk_path = pathlib.Path(UPLOADS_ROOT) / "stationpm" / station_id / report_id / "pre" / group / fname
    ...
    for group, items in photos_post.items():
        for item in (items or []):
            ...
            if not fname:
                continue
            disk_path = pathlib.Path(UPLOADS_ROOT) / "stationpm" / station_id / report_id / "post" / group / fname
