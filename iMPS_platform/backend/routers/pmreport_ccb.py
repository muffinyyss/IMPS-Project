...
async def ccbpmreport_post_submit(
    ...
    if body.job is not None:
        header["job"] = {**body.job, "pm_type": str(body.job.get("pm_type") or "CC").upper()}
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="pm_date must be YYYY-MM-DD")
    pm_type = str((header.get("job") or {}).get("pm_type") or "CC").upper()
    ...
    if link:
        ...
        issue_id, doc_name = await _resolve_issue_and_doc(
            coll, get_ccbpmurl_coll_upload(station_id), coll.database, station_id, pm_type, d,
...
@router.post("/ccbpmreport/{report_id}/approve")
async def ccbpmreport_approve(
    report_id: str,
    station_id: str = Query(...),
    current: UserClaims = Depends(get_current_user),
):
    ...
    pdf_result = await pm_pdf.ensure_report_pdf("ccb", station_id, report_id)
    ...
    maximo_result = await pm_maximo_out.safe_sync_closed(
        coll, oid, fresh, memo=f"closed by {current.username}", kind="ccb"
    )
    return {**result, "pdf": pdf_result, "maximo": maximo_result, "progress": progress}


@router.post("/ccbpmreport/{report_id}/reject")
async def ccbpmreport_reject(
    report_id: str,
