...
async def _cbboxpmreport_latest_core(station_id: str, current: UserClaims):
    ...
    pm_latest = await _latest_pm_date_from_cbboxpmreport(station_id)
    ...
    return {
        ...
        "timestamp_th": ts_th,
        "source": "stations + CBBOXPMReportDB",
    }
...
@router.get("/cbboxpmreport/latest/{station_id}")
async def cbboxpmreport_latest_by_path(
    station_id: str = Path(..., description="Station ID"),
    current: UserClaims = Depends(get_current_user),
):
    """Get latest PM info for a CB-BOX by station_id (path param)"""
    return await _cbboxpmreport_latest_core(station_id, current)
...
async def cbboxpmreport_post_submit(
    body: CBBOXPMPostIn,
    current: UserClaims = Depends(get_current_user),
):
    """Submit Post-PM report for a CB-BOX"""
    ...
    if body.job is not None:
        header["job"] = {**body.job, "pm_type": str(body.job.get("pm_type") or "CB").upper()}
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="pm_date must be YYYY-MM-DD")
    pm_type = str((header.get("job") or {}).get("pm_type") or "CB").upper()
    ...
    if link:
        ...
        issue_id, doc_name = await _resolve_issue_and_doc(
            coll, get_cbboxpmurl_coll_upload(station_id), coll.database, station_id, pm_type, d,
...
async def cbboxpmreport_upload_post_photos(
    report_id: str,
    station_id: str = Form(...),
    group: str = Form(...),
...
async def cbboxpmurl_upload_files(
    ...
    for f in files:
        ...
        urls.append(f"/uploads/cbboxpmurl/{station_id}/{pm_date}/{fname}")
...
async def cbboxpmurl_list(
    ...
    """List PM URL documents for a CB-BOX"""
    coll = get_cbboxpmurl_coll_upload(station_id)
    skip = (page - 1) * pageSize
...
@router.post("/cbboxpmreport/migrate/has-photos")
...
@router.get("/cbboxpmreport/{report_id}/photos/zip")
async def download_photos_zip(
    ...
    for group, items in photos_pre.items():
        for item in (items or []):
            ...
            if not fname:
                continue
            disk_path = pathlib.Path(UPLOADS_ROOT) / "cbboxpm" / station_id / report_id / "pre" / group / fname
    ...
    for group, items in photos_post.items():
        for item in (items or []):
            ...
            if not fname:
                continue
            disk_path = pathlib.Path(UPLOADS_ROOT) / "cbboxpm" / station_id / report_id / "post" / group / fname
