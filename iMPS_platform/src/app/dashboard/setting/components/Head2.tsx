...
const T = {
    ...
    editingCurrent: { th: "กำลังแก้ค่า Current (Power ถูกล็อกไว้)", en: "Editing Current (Power locked)" },
    editingPower: { th: "กำลังแก้ค่า Power (Current ถูกล็อกไว้)", en: "Editing Power (Current locked)" },
    lockedWhilePower: { th: "ล็อกไว้ชั่วคราว (กำลังแก้ Power)", en: "Temporarily locked (editing Power)" },
    lockedWhileCurrent: { th: "ล็อกไว้ชั่วคราว (กำลังแก้ Current)", en: "Temporarily locked (editing Current)" },
    ...
    confirmStart: {
        th: "คุณต้องการเริ่มชาร์จหัว2จริงหรือไม่?\n\nกด OK เพื่อยืนยัน หรือ Cancel เพื่อยกเลิก",
        en: "Do you really want to start charging on Head 2?\n\nPress OK to confirm or Cancel to abort.",
    },
    confirmStop: {
        th: "คุณต้องการหยุดชาร์จหัว2จริงหรือไม่?\n\nกด OK เพื่อยืนยัน หรือ Cancel เพื่อยกเลิก",
        en: "Do you really want to stop charging on Head 2?\n\nPress OK to confirm or Cancel to abort.",
    },
    chargerHead: { th: "หัวชาร์จ 2", en: "Charger Head 2" },
...
function LimitRow({
    ...
    const onBlockAttempt = () => { if (disabled) console.log(`[LimitRow] "${label}" blocked: slider is disabled.`); };

    return (
        <div className="tw-space-y-2">
            <div className="tw-flex tw-items-end tw-justify-between">
                <span className="tw-text-sm tw-text-blue-gray-700">{label}</span>
                <span className={`tw-text-sm tw-font-semibold tw-text-blue-gray-900 ${disabled ? "tw-opacity-60" : ""}`}>{value} {unit}</span>
            </div>
            <input
                type="range" min={min} max={max} step={1} value={value} disabled={disabled}
                onChange={(e) => { if (!disabled) { const v = Number(e.target.value); console.log(`[LimitRow] "${label}" changed →`, v, unit); onChange(v); } }}
                onMouseDown={onBlockAttempt} onTouchStart={onBlockAttempt}
                style={{ background: `linear-gradient(to right, ${fillColor} 0%, ${fillColor} ${percent}%, ${trackColor} ${percent}%, ${trackColor} 100%)`, cursor: disabled ? "not-allowed" : "pointer" }}
                className={["tw-w-full tw-appearance-none tw-h-2 tw-rounded-full", "tw-bg-transparent", disabled ? "tw-opacity-60" : ""].join(" ")}
            />
        </div>
    );
}
...
type PLCSetting = { SN: string; dynamic_max_current2: number; dynamic_max_power2: number; cp_status2: "start" | "stop"; };
...
function PrimaryCTA({ status, busy, onStart, onStop, lang }: { status: ChargeState; busy?: boolean; onStart: () => void; onStop: () => void; lang: Lang; }) {
    ...
    return (
        <button type="button" disabled={isDisabled} onClick={isCharging ? onStop : onStart} className={`${base} ${color} ${isDisabled ? "tw-opacity-60 tw-cursor-not-allowed" : ""}`} title={label} aria-label={label}>
...
const toNum = (v: any): number | null => { if (v === null || v === undefined) return null; const n = typeof v === "number" ? v : Number(v); return Number.isFinite(n) ? n : null; };
...
type SettingDoc = { _id: string; timestamp?: string; CP_status2?: string | number; SOC2?: string | number | null; dynamic_max_current2?: string | number; dynamic_max_power2?: string | number; present_current2?: string | number; present_power2?: string | number; [key: string]: any; };
...
export default function Head2() {
    ...
    const [maxCurrentH2, setMaxCurrentH2] = useState(0);
    const [maxPowerH2, setMaxPowerH2] = useState(0);
    ...
    const isDirty = maxCurrentH2 !== lastSaved.maxCurrentH2 || maxPowerH2 !== lastSaved.maxPowerH2;
    ...
    async function applySettings() {
        ...
        try {
            const changedMAX: Record<string, number | string> = { SN };
            if (activeLimiter === "current") { if (isDirtyCurrent) changedMAX["dynamic_max_current2"] = maxCurrentH2; }
            else if (activeLimiter === "power") { if (isDirtyPower) changedMAX["dynamic_max_power2"] = maxPowerH2; }
            else { if (isDirtyCurrent) changedMAX["dynamic_max_current2"] = maxCurrentH2; if (isDirtyPower) changedMAX["dynamic_max_power2"] = maxPowerH2; }
            const hasChange = ("dynamic_max_current2" in changedMAX) || ("dynamic_max_power2" in changedMAX);
            if (hasChange) {
                const resMAX = await fetch(`${API_BASE}/setting/PLC/MAXH2`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(changedMAX) });
                if (!resMAX.ok) throw new Error(`MAX failed: ${resMAX.status} ${await resMAX.text().catch(() => "")}`);
                setLastSaved((prev) => ({ maxCurrentH2: ("dynamic_max_current2" in changedMAX) ? maxCurrentH2 : prev.maxCurrentH2, maxPowerH2: ("dynamic_max_power2" in changedMAX) ? maxPowerH2 : prev.maxPowerH2 }));
            }
            if (cpCmd2) {
                const resCP = await fetch(`${API_BASE}/setting/PLC/CPH2`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ SN, cp_status2: cpCmd2 }) });
    ...
    const presentCurrent2 = useMemo(() => toNum(data?.present_current2), [data?.present_current2]);
    const maxCurrentSlider = useMemo(() => { const n = presentCurrent2; return n !== null ? Math.max(1, Math.round(n)) : 500; }, [presentCurrent2]);
    useEffect(() => { setMaxCurrentH2((v) => Math.min(v, maxCurrentSlider)); }, [maxCurrentSlider]);

    const presentPowerW2 = useMemo(() => {
        const p = toNum(data?.present_power2); if (p !== null) return p;
        const v = toNum((data as any)?.measured_voltage2); const i = toNum(data?.present_current2);
        if (v !== null && i !== null) return v * i; return null;
    }, [data?.present_power2, (data as any)?.measured_voltage2, data?.present_current2]);

    const maxPowerSlider = useMemo(() => { if (presentPowerW2 === null) return 500; return Math.max(1, Math.round(presentPowerW2 / 1000)); }, [presentPowerW2]);
    useEffect(() => { setMaxPowerH2((v) => Math.min(v, maxPowerSlider)); }, [maxPowerSlider]);

    useEffect(() => {
        const snFromUrl = searchParams.get("SN");
        if (snFromUrl) { setSN(snFromUrl); localStorage.setItem("selected_sn", snFromUrl); return; }
    ...
    useEffect(() => {
        ...
        const curA = toNum(data.dynamic_max_current2); if (curA !== null) setMaxCurrentH2(Math.round(curA));
        const powW = toNum(data.dynamic_max_power2); if (powW !== null) setMaxPowerH2(Math.round(powW / 1000));
        if (!initSavedRef.current) { setLastSaved({ maxCurrentH2: curA !== null ? Math.round(curA) : lastSaved.maxCurrentH2, maxPowerH2: powW !== null ? Math.round(powW / 1000) : lastSaved.maxPowerH2 }); initSavedRef.current = true; }
    }, [data]);

    const soc2 = useMemo(() => { const n = toNum(data?.SOC2); if (n === null) return 0; return Math.max(0, Math.min(100, n)); }, [data]);

    async function sendCpCommand(action: "start" | "stop") {
        if (!SN) throw new Error("no SN");
        const res = await fetch(`${API_BASE}/setting/PLC/CPH2`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ SN, cp_status2: action }) });
        if (!res.ok) { const t = await res.text().catch(() => ""); throw new Error(`CP ${action} failed: ${res.status} ${t}`); }
        return res.json();
    }

    const startH2 = async () => {
        if (!window.confirm(t("confirmStart", lang))) return;
        try { setBusyH2(true); setCpCmd2("start"); if (h2Status !== "preparing") return; await sendCpCommand("start"); } catch (e) { console.error(e); setH2Status("faulted"); } finally { setBusyH2(false); }
    };

    const stopH2 = async () => {
        if (!window.confirm(t("confirmStop", lang))) return;
        try { setBusyH2(true); setCpCmd2("stop"); await sendCpCommand("stop"); } catch (e) { console.error(e); setH2Status("faulted"); } finally { setBusyH2(false); }
    };

    const hasStation = !!SN; const hasData = !!data;
    const dynMaxCurrent2 = toNum(data?.dynamic_max_current2);
    ...
    return (
        <Card 
        // title={<div className="tw-flex tw-items-center tw-justify-between tw-gap-3"><span>Head2</span>{lastUpdated && <span className="tw-text-xs !tw-text-blue-gray-500">อัปเดตล่าสุด: {lastUpdated}</span>}</div>}
        >
            ...
            <div className="tw-space-y-8">
                <div className="tw-space-y-6">
                    <LimitRow label="Dynamic Max Current H2" unit="A" value={maxCurrentH2} onChange={(v) => { if (isGlobalDisabled) return; if (activeLimiter === null) setActiveLimiter("current"); if (activeLimiter === null || activeLimiter === "current") setMaxCurrentH2(v); }} min={0} max={maxCurrentSlider} disabled={isGlobalDisabled || disableCurrent || (activeLimiter !== null && activeLimiter !== "current")} />
                    <div className="tw-flex tw-justify-between tw-items-center tw-mt-1">
                        ...
                        <button type="button" onClick={() => { if (isGlobalDisabled) return; setMaxCurrentH2(lastSaved.maxCurrentH2); setActiveLimiter(null); }} disabled={isGlobalDisabled || activeLimiter !== "current"} className={["tw-inline-flex tw-items-center tw-gap-1 tw-text-xs tw-font-semibold tw-rounded-md tw-px-2 tw-py-1 tw-border tw-border-blue-gray-200 hover:tw-bg-blue-gray-50", (isGlobalDisabled || activeLimiter !== "current") ? "tw-opacity-60 tw-cursor-not-allowed" : ""].join(" ")}>
                            <ArrowPathIcon className="tw-w-3 tw-h-3" />{t("resetCurrent", lang)}
                        </button>
                    </div>
                    <LimitRow label="Dynamic Max Power H2" unit="kW" value={maxPowerH2} onChange={(v) => { if (isGlobalDisabled) return; if (activeLimiter === null) setActiveLimiter("power"); if (activeLimiter === null || activeLimiter === "power") setMaxPowerH2(v); }} min={0} max={maxPowerSlider} disabled={isGlobalDisabled || disablePower || (activeLimiter !== null && activeLimiter !== "power")} />
                    <div className="tw-flex tw-justify-between tw-items-center tw-mt-1">
                        ...
                        <button type="button" onClick={() => { if (isGlobalDisabled) return; setMaxPowerH2(lastSaved.maxPowerH2); setActiveLimiter(null); }} disabled={isGlobalDisabled || activeLimiter !== "power"} className={["tw-inline-flex tw-items-center tw-gap-1 tw-text-xs tw-font-semibold tw-rounded-md tw-px-2 tw-py-1 tw-border tw-border-blue-gray-200 hover:tw-bg-blue-gray-50", (isGlobalDisabled || activeLimiter !== "power") ? "tw-opacity-60 tw-cursor-not-allowed" : ""].join(" ")}>
                ...
                <HeadRow title={t("chargerHead", lang)} status={h2Status} busy={busyH2} soc={soc2 ?? 0} onStart={startH2} onStop={stopH2} lang={lang} />
            </div>
        </Card>
    );
}