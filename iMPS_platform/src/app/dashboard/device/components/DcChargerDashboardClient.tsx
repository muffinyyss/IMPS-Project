...
type Device = {
    ...
    status: Status;
    imageUrl?: string;
    metricType?: MetricType; // แสดง Times/Hour บนป้ายเล็ก
};
...
/* แปลง "15637 h 28 m" -> สตริงตามเดิม (หรือจะคำนวณนาทีก็ได้) */
...
/* ประเมินสถานะ quick */
function pickStatus(value: number | string | undefined, warnAt?: number, errorAt?: number): Status {
            ...
            {open && (
                <div className="tw-p-4">
                    <div className="tw-grid tw-gap-4 sm:tw-grid-cols-2 lg:tw-grid-cols-3 xl:tw-grid-cols-4">
                        ...
                        {filtered.length === 0 && (
                            <div className="tw-col-span-full tw-text-center tw-text-sm tw-text-gray-500 tw-py-6">ไม่พบรายการในกลุ่มนี้</div>
                        )}
                    </div>
                </div>
            )}
...
/* =========================
   ...
   ========================= */
export default function DCChargerDashboard() {
    const [live, setLive] = useState<any | null>(null); // payload ล่าสุดจาก SSE
    const esRef = useRef<EventSource | null>(null);

    // 👉 ปรับ 2 ตัวนี้ให้ตรงกับของจริง
    const API_BASE = "http://localhost:8000";
    const STATION_ID = "Klongluang3";

    useEffect(() => {
        // ต้อง login มาก่อนเพื่อมี cookie "access_token" จาก backend
        ...
        es.onerror = () => {
            // ปล่อยให้ browser จัดการ reconnect เอง (หรือจะปิด/เปิดใหม่ก็ได้)
        };
    ...
    const { LEFT_LIST, RIGHT_LIST, CENTER_LIST, DEVICES } = useMemo(() => {
        ...
        const left: Device[] = [
            {
                id: "dc-contact-l",
                name: "DC Power Contactor1",
                value: String(p.DC_power_contractor1 ?? ""),
                status: pickStatus(p.DC_power_contractor1, 30, 40),
                metricType: "times",
            },
            {
                id: "dc-contact-2",
                name: "DC Power Contactor2",
                value: String(p.DC_power_contractor2 ?? ""),
                status: pickStatus(p.DC_power_contractor2, 30, 40),
                metricType: "times",
            },
            {
                id: "dc-contact-5",
                name: "DC Power Contactor5",
                value: String(p.DC_power_contractor5 ?? ""),
                status: pickStatus(p.DC_power_contractor5, 30, 40),
                metricType: "times",
            },
            {
                id: "dc-contact-6",
                name: "DC Power Contactor6",
                value: String(p.DC_power_contractor6 ?? ""),
                status: pickStatus(p.DC_power_contractor6, 30, 40),
                metricType: "times",
            },
            {
                id: "fuse1-l",
                name: "FUSE1",
                value: parseTimes(p.FUSE1),
                status: "ok",
                metricType: "hour"
            },
            { 
            ...
            {
                id: "ac-contact-l",
                name: "AC Power Contactor1",
                value: String(p.AC_power_contractor1 ?? ""),
        ...
        const right: Device[] = [
            {
                id: "dc-contact-3",
                name: "DC Power Contactor3",
                value: String(p.DC_power_contractor3 ?? ""),
                status: pickStatus(p.DC_power_contractor3, 30, 40),
                metricType: "times",
            },
            {
                id: "dc-contact-4",
                name: "DC Power Contactor4",
                value: String(p.DC_power_contractor4 ?? ""),
                status: pickStatus(p.DC_power_contractor4, 30, 40),
                metricType: "times",
            },
            {
                id: "fuse2-2",
                name: "FUSE2",
                value: parseTimes(p.FUSE2),
                status: "ok",
                metricType: "hour"
            },
            { 
            ...
            {
                id: "ac-contact-2",
                name: "AC Power Contactor2",
                value: String(p.AC_power_contractor2 ?? ""),
        ...
        // รวมทั้งหมดสำหรับแผงสรุปด้านล่าง
        const all: Device[] = [...left, ...right];
    ...
    return (
        <div className="tw-mx-auto tw-max-w-[1600px] tw-w-full tw-pt-8 md:tw-pt-10">
            ...
            <div className="tw-mt-6 tw-rounded-3xl tw-bg-white tw-shadow-sm tw-ring-1 tw-ring-black/5 tw-p-4">
                <SideList title="อุปกรณ์" items={CENTER_LIST} />
            </div>
            ...
            <div className="tw-mt-8 tw-space-y-6">
                {(filter === "all" || filter === "error") && (
                    <Group status="error" title="อุปกรณ์ที่มีปัญหา" devices={DEVICES.filter((d) => d.status === "error")} defaultOpen search={query} />
                )}
                {(filter === "all" || filter === "warn") && DEVICES.some((d) => d.status === "warn") && (
                    <Group status="warn" title="อุปกรณ์ที่ต้องตรวจสอบ" devices={DEVICES.filter((d) => d.status === "warn")} defaultOpen search={query} />
                )}
                {(filter === "all" || filter === "ok") && (
                    <Group status="ok" title="อุปกรณ์สถานะปกติ" devices={DEVICES.filter((d) => d.status === "ok")} defaultOpen={false} search={query} />
                )}
            </div>
