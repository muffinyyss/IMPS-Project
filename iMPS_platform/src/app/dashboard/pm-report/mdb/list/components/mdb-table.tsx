  ...
  const columns: ColumnDef<TData, unknown>[] = useMemo(() => [
    ...
    },
    {
      ...
      cell: (info: CellContext<TData, unknown>) => {
        ...
        // ใบค้างจากตอนที่ฟอร์มยังมี 2 ด่าน — เลขข้อคนละชุดกับฟอร์มปัจจุบัน
        // กรอกต่อในฟอร์มใหม่ไม่ได้ เปิดดูข้อมูลที่เคยกรอกไว้ได้อย่างเดียว
        if (rowSide == "pre") {
          return (
            <div className="tw-flex tw-items-center tw-justify-center">
              <Button size="sm" color="blue" variant="outlined"
                ...
                onClick={() => {
                  const params = new URLSearchParams(searchParams.toString());
                  params.set("view", "form"); params.set("edit_id", info.row.original.id || "");
                  params.set("review", "1");
                  router.push(`${pathname}?${params.toString()}`, { scroll: false });
                }}>{t("viewReport", lang)}</Button>
            </div>
          );
        } else {
          return (
              ...
              </a>
              {
                info.row.original.has_photos && (
                  <a
                    ...
                    href={`${BASE}/mdbpmreport/${info.row.original.id}/photos/zip?station_id=${encodeURIComponent(stationId || "")}`}
                    target="_blank"
                    rel="noopener noreferrer"
