...
export default function SearchDataTables({ token, apiBase = BASE }: Props) {
  ...
  const { wos, reloadWos, canApprove, canPlan, flowTab, setFlowTab } = usePmFlow("ccb", stationId);
  ...
  const onApprove = async () => {
    ...
    try {
      const res = await apiFetch(
        `${apiBase}/ccbpmreport/${encodeURIComponent(approveRow.id)}/approve?station_id=${encodeURIComponent(stationId)}`,
  ...
  const onReject = async () => {
    ...
    try {
      const res = await apiFetch(
        `${apiBase}/ccbpmreport/${encodeURIComponent(rejectRow.id)}/reject?station_id=${encodeURIComponent(stationId)}`,
  ...
  if (mode === "form") {
    ...
    if (woInfoWonum) {
      return (
        <PmWorkOrderInfo
          source="ccb" identifier={stationId} wonum={woInfoWonum}
    ...
    if (planningWonum) {
      return (
        <PmPlanForm
          source="ccb" identifier={stationId} wonum={planningWonum}
          onSaved={() => { showToast("success", t("planSaved", lang)); reloadWos(); leavePlanning(); }}
          onCancel={leavePlanning}
        />
      );
    }
    return (<div className="tw-mt-4 sm:tw-mt-6 lg:tw-mt-8"><CCBPMForm /></div>);
  }
