...
export default function StatisticsCards() {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      if (!stationId) { setPm(fallback); setErr(null); return; }
      try {
        setLoading(true);
        setErr(null);
        const token = typeof window !== "undefined" ? localStorage.getItem("access_token") ?? undefined : undefined;
        const data = await loadPmReport(stationId, token);
        if (alive) setPm(data);
      } catch (e: any) {
        if (alive) { setPm(fallback); setErr(e?.message ?? "โหลดข้อมูลไม่สำเร็จ"); }
      } finally {
  ...
  return (
    <>
      {!stationId && <p className="tw-text-sm tw-text-blue-gray-500 tw-mt-2">เลือกสถานีจากแถบด้านบนเพื่อแสดง PM Report</p>}
