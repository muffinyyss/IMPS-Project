...
export default function SearchDataTables({ token, apiBase = BASE }: Props) {
  ...
  const { wos, reloadWos, canApprove, canPlan, flowTab, setFlowTab } = usePmFlow("station", stationId);
  ...
  const onApprove = async () => {
    ...
    try {
      const res = await apiFetch(
        `${apiBase}/stationpmreport/${encodeURIComponent(approveRow.id)}/approve?station_id=${encodeURIComponent(stationId)}`,
  ...
  const onReject = async () => {
    ...
    try {
      const res = await apiFetch(
        `${apiBase}/stationpmreport/${encodeURIComponent(rejectRow.id)}/reject?station_id=${encodeURIComponent(stationId)}`,
