...
export default function HealthIndex({
  ...
  return (
    <Card className={`tw-border tw-border-blue-gray-100 tw-shadow-sm ${className ?? ""}`}>
      <CardHeader floated={false} shadow={false} className="tw-px-4 tw-py-3 tw-flex tw-items-center tw-justify-between">
            ...
            {Array.from({ length: segments }).map((_, i) => (
              <div
                ...
                className={`tw-h-4 tw-flex-1 tw-rounded-[2px] ${
                  i < filled
                    ? v >= 80 ? "tw-bg-green-500" : v >= 50 ? "tw-bg-amber-500" : "tw-bg-red-500"
                    : "tw-bg-blue-gray-100"
                }`}
              />
            ))}
