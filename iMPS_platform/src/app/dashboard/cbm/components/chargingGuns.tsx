...
type GunItem = {
    id: string;
    name: string;     // "Charging Gun 1 Temperature"
    temp: number;     // °C
...
type GunsProps = {
    title?: string;
    updatedAt?: string;
    items: GunItem[]; // ส่งมา 2 รายการ: Gun 1 / Gun 2
};

/* ---------- utils ---------- */
function toneByTemp(temp: number) {
    if (temp > 65)
        return {
            bar: "tw-bg-red-500",
...
/* ---------- tile ---------- */
function GunTile({ name, temp, target = 60 }: { name: string; temp: number; target?: number }) {
    const tone = toneByTemp(temp);
...
/* ---------- card ---------- */
export default function ChargingGunsCard({
    title = "Charging Guns Temperature",
    updatedAt,
    items,
}: GunsProps) {
    return (
            ...
            <div className="tw-grid tw-gap-4 tw-grid-cols-1 sm:tw-grid-cols-2 [&>div]:tw-h-full">
                {items.length === 0 ? (
                    <div className="tw-col-span-full tw-text-center tw-text-sm tw-text-blue-gray-400 tw-py-6">
                        No charging gun data
                    </div>
                ) : (
                    items.map((it) => (
                        <GunTile key={it.id} name={it.name} temp={it.temp} target={it.target ?? 60} />
                    ))
                )}
            </div>
        </section>
