...
/** TabId = ใช้ใน UI | ApiType = ส่งเข้า list/form/backend */
export type TabId = "charger" | "mdb" | "ccb" | "cb-box" | "station";
export type ApiType = "charger" | "mdb" | "ccb" | "cb_box" | "station";
...
export default function DataTablesPage({ params }: { params: { type?: string } }) {
  // อ่าน type จาก path และ fallback ถ้าไม่แมตช์
  const segment = String(params.type || "").toLowerCase() as TabId;
  const initialTab: TabId = (ALLOWED as readonly string[]).includes(segment) ? segment : "charger";

  const [active, setActive] = useState<TabId>(initialTab);

  // ถ้าเปลี่ยนพารามิเตอร์ (เช่นผู้ใช้รีโหลดหน้าอื่น) ให้ sync ครั้งแรกเท่านั้น
  useEffect(() => {
    if ((ALLOWED as readonly string[]).includes(segment) && segment !== active) {
      setActive(segment);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [segment]);

  // เลิกใช้ router.replace ทั้งหมด — เปลี่ยนแท็บด้วย state อย่างเดียว
  const handleChange = (nextValue: string) => {
    setActive(nextValue as TabId);
  };
  ...
  return (
    <Tabs id="data-tabs" value={active} onChange={handleChange} className="tw-w-full">
      ...
      >
        {TABS.map(({ id, apiType }) => (
          <TabPanel key={id} value={id} className="tw-p-0">
            {/* ภายใน ListCMTables/CMForm คุณได้ใส่ type='button' และกัน submit แล้ว */}
            <ListCMTables equipmentType={apiType} />
          </TabPanel>
        ))}
      </TabsBody>
