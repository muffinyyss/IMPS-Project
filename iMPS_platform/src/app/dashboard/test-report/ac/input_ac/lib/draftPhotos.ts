// app/dashboard/test-report/ac/input_ac/lib/draftPhotos.ts
import { createStore, set, get, del, keys } from "idb-keyval";

const store = createStore("acDraftDB", "photos");
...
export function makePhotoDbKey(draftKey: string, photoId: string) {
  return `acDraftPhoto:${draftKey}:${photoId}`;
}
...
export async function clearPhotosForDraft(draftKey: string) {
  try {
    ...
    const prefix = `acDraftPhoto:${draftKey}:`;
...
export async function clearAllPhotos() {
  try {
    const allKeys = await keys(store);
    const acKeys = allKeys.filter(
      (k) => typeof k === "string" && k.startsWith("acDraftPhoto:")
    );
    await Promise.all(acKeys.map((k) => del(k, store)));
  } catch (e) {
