...
import CMForm from "@/app/dashboard/test-report/ac/input_ac/checkList";
...
const buildDocumentName = (it: any, isoDay: string): string => {
  ...
  if (issueId) return `AC Test Report - ${issueId}`;
  if (isoDay) return `AC Test Report - ${isoDay}`;
  return "AC Test Report";
};
...
export default function CMReportPage({ token, apiBase = BASE }: Props) {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const [acRes, urlRes] = await Promise.allSettled([
      ...
      if (acRes.status === "fulfilled" && acRes.value.ok) {
        const j = await acRes.value.json();
        if (Array.isArray(j?.items)) acItems = j.items;
      }
      ...
      const acRows: TData[] = acItems.map((it: any) => {
      ...
      const urlRows: TData[] = urlItems.map((it: any) => {
        const isoDay = toISODateOnly(it.ac_date ?? it.reportDate ?? it.createdAt ?? "");
        const raw =
          it.file_url
          ?? (Array.isArray(it.urls) ? (it.urls[0]?.url ?? it.urls[0]) : it.url)
          ?? it.file
      ...
      const allRows = [...acRows, ...urlRows].sort((a, b) => {
        const da = a.acDateISO ?? "";
        const db = b.acDateISO ?? "";
