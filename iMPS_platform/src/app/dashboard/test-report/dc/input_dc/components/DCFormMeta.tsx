...
export const ValidationSummary: React.FC<ValidationSummaryProps> = ({ head, lang }) => {
  ...
  const scrollToField = (fieldKey: string) => {
    ...
    setTimeout(() => {
      ...
      if (element) {
        ...
        // Use inline style for more reliable highlighting
...
export default function DCFormMeta({ head, onHeadChange }: DCFormMetaProps) {
  ...
  // ★★★ Check if firmware has value from API ★★★
  const firmwareFromApi = hasFirmwareValue(head.firmware_version) && !userEditedFirmware.current;
          ...
          {firmwareFromApi ? (
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                onHeadChange({ firmware_version: e.target.value });
              }}
              crossOrigin=""
              className="!tw-w-full"
              containerProps={{ className: "!tw-min-w-0" }}
