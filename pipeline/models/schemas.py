...
def create_cbm_document(state: 'StationState', cbm_data: Dict[str, Any],
    ...
    doc = {
        ...
        "router_temp": parse_float(cbm_data.get("Router", {}).get("rt_temp", 0)) / 10 if cbm_data.get("Router", {}).get("rt_temp") else 0,
        "RSSI_router": cbm_data.get("Router", {}).get("rssi"),
    ...
    # มี sensor จริง 2 ตัว (tempPowerModule1/2) แบ่งตามหัวชาร์จเหมือน UI:
    # หัว 1 = ครึ่งแรก -> tempPowerModule1, หัว 2 = ที่เหลือ -> tempPowerModule2
    ...
    for i in range(1, pm_count + 1):
        ...
        doc[f"power_module_temp{i}"] = parse_int(plc_data.get(sensor))
...
def create_module3_document(state: 'StationState', mdb_data: Dict[str, Any],
    ...
    return {
        ...
        "RSSI": cbm_data.get("Router", {}).get("rssi"),
